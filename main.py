...
for tweet_id, topic, number, language, tweets in all_tweets:
    ...
    # Check if current user already rated this tweet
    c.execute('SELECT rating FROM ratings WHERE tweet_id=? AND user_id=?', (tweet_id, st.session_state['user_id']))
        ...
        if rated:
            ...
            if st.button("👍 Like", key=f"like_{tweet_id}"):
                try:
                    ...
                    pass
                st.info("Thank you for your like!")
    with col2:
        if rated:
